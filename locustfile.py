...
class TrafficLoad(HttpUser):
    ...
    def get_sensor_data(self):
        ...
        for sensor_type in sensor_types:
            for limit in limits:
                self.client.get(f"http://192.168.100.15:31145/api/data?type={sensor_type}&limit={limit}")
    ...
    def get_min_max_data(self):
        ...
        for sensor_type in sensor_types:
            for limit in limits:
                self.client.get(f"http://192.168.100.15:31145/api/data/min_max?type={sensor_type}&limit={limit}")
    ...
    def export_csv_data(self):
        ...
        for sensor_type in sensor_types:
            for limit in limits:
                self.client.get(f"http://192.168.100.15:31145/api/data/export_to_csv?type={sensor_type}&limit={limit}")